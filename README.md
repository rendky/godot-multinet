...
Every system must own its state! this would make them publish immutable snapshots, and fail elegantly when it has to.

- Things considered **Canon** to Multinet must own  event ordering, idempotency, recovery metadata... If canon is confused, the world is confused.
...
- **CADENCE (proprietary motion system thinned out for public use)** does locomotion, gait, pose, and contact expectation using pure magic (data).
...
- **LivingWorldRendering (LWR)**; our renderer, is native to multinet actually. It's just named for eccentricism, but it handles representations, direct lighting, and final pixel stuff. 

---

## Platform Floors

Multinet is being designed to run on humble hardware without stuttering. Our target is:

- **Portable Floor:** Samsung A21s, iPhone 7 Plus.
- **Desktop Floor:** GTX 860M 2GB, i7-4710HQ, 16GB DDR3 RAM, 5400 RPM HDD.

### Hard Limits we'll be enforcing before it bites us. 
...
If a feature causes frame stalls on the portable floor which is our lowest floor tier, it calls for a redesign.

---

## Regarding DETERMINISM. we consider it very important.

Procedural generation and network validation rely on SquirrelNoise5 as our primary integer hash because we love it and it's fast enough. 
...
# Build multinet for faster iteration
run build_multinet.bat
```
